package com.greencom.android.podcasts2.ui.common

import androidx.compose.material.icons.Icons
import androidx.compose.material.icons.outlined.Add
import androidx.compose.material.icons.outlined.Close
import androidx.compose.material.icons.outlined.Done
import androidx.compose.material.icons.outlined.Search
import com.greencom.android.podcasts2.R

object PodcastsIcons {
    const val PodcastsFilled = R.drawable.ic_podcasts_filled_24
    const val PodcastsOutlined = R.drawable.ic_podcasts_outlined_24
    const val DiscoverFilled = R.drawable.ic_discover_filled_24
    const val DiscoverOutlined = R.drawable.ic_discover_outlined_24
    const val LibraryFilled = R.drawable.ic_library_filled_24
    const val LibraryOutlined = R.drawable.ic_library_outlined_24

    val Search = Icons.Outlined.Search
    val Done = Icons.Outlined.Done
    val Add = Icons.Outlined.Add
    val Close = Icons.Outlined.Close
}
